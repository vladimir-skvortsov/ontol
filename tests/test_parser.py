...
def test_parse_logical_expression(parser):
    ...
    assert ontology.logical_expressions[0].expression == "element1 in set1 and element2 in set2"
...
def test_parse_comments(parser):
    ...
    ontology = parser.parse(content)
    assert len(ontology.comments) == 2
    assert ontology.comments[0].text == "Это комментарий"
    assert ontology.comments[1].text == "Еще один комментарий"
