...
def test_function_creation():
    func = Function(name="TestFunction", input_types=["int", "str"], output_types=["bool"], description="A test function")
    ...
    assert repr(func) == "Function(name=TestFunction, input_types=['int', 'str'], output_types=['bool'], label=None, description=A test function)"
    
    func_no_desc = Function(name="NoDescFunction", input_types=["float"], output_types=["str"])
    assert func_no_desc.description is None
    assert repr(func_no_desc) == "Function(name=NoDescFunction, input_types=['float'], output_types=['str'], label=None, description=None)"
    
    func_with_label = Function(name="LabeledFunction", input_types=["int"], output_types=["str"], label="SomeLabel")
    assert func_with_label.label == "SomeLabel"
    assert repr(func_with_label) == "Function(name=LabeledFunction, input_types=['int'], output_types=['str'], label=SomeLabel, description=None)"
...
def test_meta_creation():
    meta = Meta(version="1.0", name="TestOntology", author="Author", description="A test ontology", date_created="2024-01-01")
    assert meta.version == "1.0"
    assert meta.name == "TestOntology"
    assert meta.author == "Author"
    assert meta.description == "A test ontology"
    assert meta.date_created == "2024-01-01"
    assert repr(meta) == "Meta(version=1.0, name=TestOntology, author=Author, description=A test ontology, date_created=2024-01-01)"
    
    meta_no_optional = Meta(version="1.0", name="TestOntology", author="Author", description=None, date_created=None)
    assert meta_no_optional.description is None
    assert meta_no_optional.date_created is None
    assert repr(meta_no_optional) == "Meta(version=1.0, name=TestOntology, author=Author, description=None, date_created=None)"
...
def test_ontology_operations():
    ...
    with pytest.raises(ValueError, match="Meta information is already set and can only be set once."):
        ontology.set_meta(meta)
    ...
    assert "Ontology(types=[Term(name=TestTerm, description=None), Term(name=NewTerm, description=None)]" in repr_str
    assert "functions=[Function(name=TestFunction, input_types=['int'], output_types=['str'], label=None, description=None), Function(name=NewFunction, input_types=['str'], output_types=['int'], label=None, description=None)]" in repr_str